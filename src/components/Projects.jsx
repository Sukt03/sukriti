...
export default function Projects() {
  ...
  return <section id="projects" className="section-shell relative scroll-mt-28 overflow-hidden py-10 sm:py-12"><NodeGraph reducedMotion={reducedMotion}/><div className="relative"><SectionHeading {...sectionMeta.projects}/><div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">{projects.map((project, index) => <RevealItem key={project.title} index={index}><article className={`h-full rounded-xl border border-transparent bg-card p-5 ${reducedMotion ? '' : 'transition-all duration-200 hover:-translate-y-0.5 hover:border-purple'}`}><h3 className="mb-2 font-medium text-purpleDeep">{project.title}</h3><p className="mb-4 text-sm leading-6 text-inkSoft">{project.description}</p><div className="mb-4 font-mono text-[0.7rem] text-pinkDeep">{project.stack}</div><a href={project.href} target="_blank" rel="noopener" className="font-mono text-xs text-purpleDeep underline decoration-purple/30 underline-offset-4 hover:decoration-pink">{project.linkLabel}</a></article></RevealItem>)}</div></div></section>
}
