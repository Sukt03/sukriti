...
function ResearchVisual({ reducedMotion }) {
  return <div className="relative mx-auto aspect-square w-full max-w-[340px]" aria-hidden="false">
    ...
    <div className="absolute inset-[15%] z-10 overflow-hidden rounded-full border-4 border-paper shadow-[0_18px_60px_rgba(92,59,112,0.22)] ring-1 ring-purple/20">
      <img src={site.hero.portraitSrc} alt={site.hero.portraitAlt} className="h-full w-full -translate-x-2 -rotate-90 scale-100 object-cover object-center"/>
    </div>
...
export default function Hero() {
  ...
  return <section className="relative overflow-hidden px-5 pb-14 pt-14 sm:px-10 sm:pb-20 sm:pt-20"><div className="absolute -left-24 top-4 h-72 w-72 rounded-full bg-purple/5 blur-3xl"/><div className="relative grid items-center gap-12 lg:grid-cols-[1.25fr_0.75fr]"><div className="max-w-3xl"><div className="mb-5 inline-flex items-center gap-2 rounded-full border border-pink/20 bg-card/60 px-3 py-1 font-mono text-xs tracking-wider text-pinkDeep"><span className="h-1.5 w-1.5 rounded-full bg-pink"/>{site.hero.eyebrow}</div><h1 className="mb-6 font-display text-5xl font-medium leading-none text-purpleDeep sm:text-7xl lg:text-8xl">{site.hero.name}</h1><div className="mb-6 h-px w-24 bg-gradient-to-r from-purple to-pink"/><p className="mb-8 max-w-2xl text-base leading-7 text-inkSoft sm:text-lg">{site.hero.lede}</p><div className="flex flex-wrap gap-3">{site.hero.actions.map((action) => <a key={action.label} href={action.href} target={action.external ? '_blank' : undefined} rel={action.external ? 'noopener' : undefined} className={`rounded-md border px-5 py-2.5 text-sm ${reducedMotion ? '' : 'transition-all duration-200'} ${action.primary ? 'border-purple bg-purple text-white hover:border-purpleDeep hover:bg-purpleDeep' : 'border-hairline text-ink hover:border-purple hover:text-purpleDeep'}`}>{action.label}</a>)}</div></div><ResearchVisual reducedMotion={reducedMotion}/></div></section>
}
